...
Offline helper page help (separate page): **offline-help.html**.

Online/server UI help (separate page): **online-help.html**.
...
Notes (online UI vs offline helper):

- The online/server UI (`/online.html`) is upload-first and is meant to run batch mode directly.
- The offline helper page (`offline.html`) is where you’ll find copy/paste-ready CLI commands (batch `run` and interactive `chat`).
...
- Export a config JSON if you want to run the exact same config in a terminal later.
...
#### Exporting a KiCad 9.0 SPICE netlist (`.cir`)

In KiCad 9.0 (Schematic Editor):

1. Open your schematic.
2. Open **Export Netlist…** and switch to the **SPICE** tab.
3. Check only **Save all voltages** and **Save all currents** (leave the other SPICE options unchecked).
4. Click **Save...** and save as `baseline.cir`.

Then pass it into the tool with `--baseline-netlist baseline.cir`.
