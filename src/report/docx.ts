...
export async function writeReportDocx(args: {
  ...
  /**
   * Summary lines for any .SUBCKT models generated by the SUBCKT integration step.
   * When present a "Generated SUBCKT Models" section is added before the SPICE netlist.
   */
  generatedSubcktModels?: string[];
}): Promise<void> {
  ...
  const subcktSection: Paragraph[] = [];
  const subcktLines = Array.isArray(args.generatedSubcktModels) ? args.generatedSubcktModels : [];
  if (subcktLines.length) {
    subcktSection.push(
      new Paragraph({ text: "Generated SUBCKT Models", heading: HeadingLevel.HEADING_1 }),
      new Paragraph(
        "The following AI-generated .SUBCKT library models were produced during this run. "
        + "Each model is a simulation approximation — verify against the datasheet before production use.",
      ),
    );
    for (const line of subcktLines) {
      subcktSection.push(new Paragraph({ text: line, bullet: { level: 0 } }));
    }
    subcktSection.push(
      new Paragraph(
        "Generated .lib files and an updated final-with-libs.cir (with .include directives) are "
        + "available in the run output directory alongside this report.",
      ),
    );
  }
