...
### 2.1 Phase 0 baseline decisions

The following Phase 0 baseline decisions are treated as finalized for this plan:

- the target direction is a protocol-driven, registry-backed, hosted-capable architecture built from the current local codebase in stages
- the MVP vs deferred split in [Section 17](#17-recommended-mvp-scope) is the working scope baseline
- breaking changes to legacy outputs are acceptable for this migration for `final.md`, `final.cir`, and `final.json`
- the informational content currently emitted in `report.docx` must be preserved; new report sections may be added, but existing report information should not be dropped
- automated migration support for existing config files and saved run folders is deferred unless a later requirement makes it necessary
- the initial canonical entity vocabulary is:
  - `ProviderDefinition`
...
This gives the migration a fixed Phase 0 vocabulary and baseline so implementation can begin without reopening core terminology.

The current `report.docx` information baseline, as generated by [src/report/docx.ts](src/report/docx.ts), includes:

- baseline schematic screenshot when present
- reference images when present
- the run question
- ensembled output rendered from final markdown
- per-model answer sections when present
- the SPICE netlist
- the connectivity schematic section/image or its fallback message

Future report evolution may add content, but this baseline information set should remain represented in the report deliverables.
